...
def sample_random_graph(key, backbone_coordinates, temperature=1.0) -> jnp.array:
    ...
    sorted_distances = jnp.take_along_axis(distances, argsorted_distances, axis=1)
    remaining_distances = sorted_distances[:, k_nearest:]
    ...
    indices_for_sorted_distance = gumbel_perturbed_argsorted[:, -k_cubic:] + 20  # top_k_cubic indices
    top_k_cubic_samples = jnp.take(argsorted_distances, indices_for_sorted_distance)
